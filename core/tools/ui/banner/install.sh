...
CORE_BANNER_MARKER="# ===== W8Core-Termux-Moded Banner ====="
...
_install_banner_impl() {
	...
	if grep -qF "$CORE_BANNER_MARKER" "$shell_config" 2>/dev/null; then
		log_info "W8Core-Termux-Moded Banner already installed"
		return 0
	fi
...
EOF
	...
	log_success "W8Core-Termux-Moded Banner installed"
...
install_banner() {
	...
	log_info "Installing W8Core-Termux-Moded Banner..."
	mkdir -p "$(dirname "$LOG_FILE")"
...
_uninstall_banner_impl() {
	...
	if ! grep -qF "$CORE_BANNER_MARKER" "$shell_config" 2>/dev/null; then
		log_warn "W8Core-Termux-Moded Banner not installed"
		return 0
	fi
	...
	if [[ -n "$marker_line" ]]; then
		...
		log_success "W8Core-Termux-Moded Banner uninstalled"
	else
...
uninstall_banner() {
	...
	log_info "Uninstalling W8Core-Termux-Moded Banner..."
...
update_banner() {
	log_info "Updating W8Core-Termux-Moded Banner..."

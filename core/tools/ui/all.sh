...
install_all_ui_components() {
	...
	for tool in "${UI_COMPONENTS[@]}"; do
		...
		cursor)
			...
			;;
		banner)
			loading "Installing W8Core-Termux-Moded Banner" install_banner
			case $? in 0) ((installed_count++));; 1) ((failed_count++));; esac
			;;
		esac
	done
...
uninstall_all_ui_components() {
	...
	for tool in "${UI_COMPONENTS[@]}"; do
		...
		cursor)
			...
			;;
		banner)
			loading "Uninstalling W8Core-Termux-Moded Banner" uninstall_banner
			case $? in 0) ((uninstalled_count++));; 1) ((failed_count++));; esac
			;;
		esac
	done
...
update_all_ui_components() {
  ...
  for tool in "${UI_COMPONENTS[@]}"; do
    ...
    cursor)
      ...
      ;;
    banner)
      loading "Updating W8Core-Termux-Moded Banner" update_banner
      case $? in 0) ((updated_count++));; 1) ((failed_count++));; esac
      ;;
    esac
  done
...
reinstall_all_ui_components() {
  ...
  for tool in "${UI_COMPONENTS[@]}"; do
    ...
    cursor)
      ...
      ;;
    banner)
      loading "Reinstalling W8Core-Termux-Moded Banner" reinstall_banner
      case $? in 0) ((reinstalled_count++));; 1) ((failed_count++));; esac
      ;;
    esac
  done
